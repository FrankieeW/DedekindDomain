...
\begindefinition{Maximal ideal}{def-maximal-ideal}
An ideal $I \subsetneq R$ is maximal if whenever
$I \subseteq J \subseteq R$, then $J = I$ or $J = R$.
Equivalently, $I$ is maximal iff $R/I$ is a field.
...
\begintheorem{Imaginary quadratic Euclidean classification (L10.4)}{thm-imaginary-euclidean}
For squarefree $d<0$, the following are equivalent:
\[
\mathcal O_d \text{ is norm Euclidean}
\Longleftrightarrow
\mathcal O_d \text{ is Euclidean}
\Longleftrightarrow
d \in \{-1,-2,-3,-7,-11\}.
\]
\uses{def-euclidean-function}
...
\begintheorem{Non-Euclidean PID examples (L10.5)}{thm-pid-not-euclidean-examples}
The rings
\[
\mathcal O_{-19},\ \mathcal O_{-43},\ \mathcal O_{-67},\ \mathcal O_{-163}
\]
are PIDs (hence examples showing PID $\centernot\Rightarrow$ Euclidean in general).
\lean{../../DedekindDomain/IntegerRing.lean}
\endtheorem

\begintheorem{Real quadratic norm Euclidean classification (L10.6)}{thm-real-norm-euclidean}
For squarefree $d>0$,
\[
\mathcal O_d \text{ is norm Euclidean}
\iff
